...
- Q: How does drain stream per-pod progress from the backend to the browser? → A: **Server-Sent Events (SSE).** The drain endpoint responds with `Content-Type: text/event-stream`; each per-pod result is emitted as a single `data: {...}\n\n` event (JSON payload `{pod, namespace, result: "evicted"|"skipped"|"error", detail}`); a final `event: complete\ndata: {summary}\n\n` closes the stream. Browser uses native `EventSource`; the backend uses Go's `http.Flusher.Flush()` after each event. SSE is one-way (server → client), which matches the drain shape exactly; no WebSocket framing tax, no polling overhead, and `EventSource` auto-reconnects on transient network blips. About 30 lines of Go to implement.
