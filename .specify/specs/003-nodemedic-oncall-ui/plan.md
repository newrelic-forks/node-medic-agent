...
**Approach (from research):** Go 1.24 + stdlib `net/http` + `html/template` for SSR; `controller-runtime`'s typed `client.Client` for direct apiserver reads and writes (no informer cache — every list/get hits the apiserver, per FR-24's no-staleness contract); browser-side `fetch()` + ReadableStream parsing of the SSE drain frames over `http.Flusher.Flush()` (POST + SSE — `EventSource` is GET-only); plain `fetch()` + `setInterval(_, 30000)` for the list-view refresh; `BackgroundConcurrency=3` for the eviction loop (tasks complete in parallel, results stream out in arrival order); fields-grid Block Kit shape (`section.fields`, two columns) with a `text` mrkdwn fallback in `attachments[].fallback`; new Helm chart at `deployment/helm/nodemedic-oncall-ui/` mirroring the agent's chart structure (`requireTestCluster` guard, per-cloud values split, ServiceAccount + ClusterRole pattern); new `Dockerfile.nodemedic-oncall-ui` (multi-stage, `linux/amd64`); image tag `cf-registry.nr-ops.net/container-fabric/nodemedic-oncall-ui:dev-cf1z-<sha>` consistent with controller + agent.
...
- `sigs.k8s.io/controller-runtime` v0.21.x — only for the **client builder + scheme registration**. The UI is not a controller, so the manager, reconciler, and informer machinery are unused; `client.New(...)` returns a typed `client.Client` that reads and writes directly against the apiserver. FR-24 binds "MUST NOT cache NHD reads beyond a single request" — no informer, no in-process cache. Same dep the controller already pulls (verified via `go.sum`).
...
**Storage**: N/A. The UI is stateless. All state lives in the apiserver — NHDs (read), Nodes (read/patch), Pods (read), the `ui-action-history` annotation on the NHD (read/patch). No SQLite, no Redis, no in-memory cache. FR-24 explicitly requires every list/detail render to hit the apiserver — chattier than an informer-backed surface, but no staleness window. Drain in-flight state is a process-local `sync.Map[node-name -> *DrainProgress]` so concurrent button clicks during a drain are coalesced (FR-9 G9 idempotency on uncordon/clear; "drain in progress" indicator on drain). The `sync.Map` is process state, not a cache of apiserver objects, so it doesn't engage FR-24.
...
**Performance Goals**:
- List view request → first byte: ≤ 400 ms p99 on cf1z (one apiserver `List NHDs` in the configured namespace, in-process filter to last 24h; no informer per FR-24). Demo volume is ~5–50 NHDs — the LIST is single-digit-ms on cf1z's apiserver.
- Per-case page request → first byte: ≤ 500 ms p99 (one apiserver `Get NHD` + one apiserver `Get Node`; both direct, no cache).
...
│       ├── kube/
│       │   └── client.go                # controller-runtime client builder, scheme registration, in-cluster config — direct typed client, no informer (FR-24)
