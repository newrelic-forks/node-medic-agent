...
- Q: Which namespace does the agent deploy to? Earlier drafts said `container-fabric`, but the controller and `hack-node-problem-detector` both live in `cf-monitoring`. → A: **`cf-monitoring`**. cf1z reality wins. The controller's `agentURL` (in its `values-azure.yaml`) is `http://nodemedic-agent.cf-monitoring.svc:8080/diagnose` — already pointing at `cf-monitoring`. Aligning the agent there means all three NodeMedic-related workloads (NPD daemon, controller, agent) co-locate in one namespace, the cross-scope HTTP contract resolves cleanly, and there's no extra cross-namespace `Service` routing to debug. Spec 002's manifests + AC-1 + FR-15 + demo flow §9 + glossary updated to `cf-monitoring` accordingly. Note: `container-fabric` still appears in §10 in the GHE org URL `source.datanerd.us/container-fabric/k8s-resource-recommendations` — that's the team's *GitHub Enterprise org*, unrelated to the kube namespace. Untouched.
- Q: What's the canonical image path for the agent? → A: **`cf-registry.nr-ops.net/container-fabric/nodemedic-agent`**, mirroring the controller's path (`cf-registry.nr-ops.net/container-fabric/nodemedic-controller`) exactly. Same registry, same `container-fabric/*` namespace prefix, parallel naming. Push works with developer cf-registry creds (verified empirically 2026-06-14 via the controller image push). Tag convention `dev-cf1z-<short-sha>` from the agent repo's branch HEAD. Gives cf1z a coherent `nodemedic-*` image family in cf-registry. NFR-9 + manifests/README updated.
...
| G11 | Run as an **independent Deployment + Service** in the same cluster *and same namespace* as the controller (not as a sidecar in the same pod, not on `interlinked`). On cf1z the namespace is `cf-monitoring` (per Clarifications). The controller reaches the agent at `http://nodemedic-agent.cf-monitoring.svc:8080/diagnose` (matches the `agentURL` in the controller's `values-azure.yaml`); the agent reads/updates the NHD CR via the in-cluster apiserver. Agent and controller have independent lifecycles, RBAC, and resource limits. | Decision bound in §3. Separate deployments mean either side can be restarted, scaled, or rolled without disturbing the other; co-locating in `cf-monitoring` matches the existing controller + `hack-node-problem-detector` deployment shape. |
...
- Construct `ClaudeAgentOptions` with:
  ...
  - `hooks = [("PreToolUse", tool_log_hook), ("PostToolUse", tool_log_hook)]` — emits a structured stdout log line per tool call (NFR-3). The `PreToolUse` fire records `tool` + `command-or-args`; the `PostToolUse` fire fills in `latency_ms`, `exit_code` (Bash), and `stdout_bytes`/`stderr_bytes` (Bash). Both fires share `case_id` and `turn` so a single tool call reconstructs as a Pre/Post pair in the structured logs.
...
| `Failed` | Do NOT overwrite. **This covers the controller-deadline race introduced by FR-7's no-agent-side-deadline rule:** the controller's deadline (Spec 001 FR-5, with one ~60 s retry extension via `RetryDeadlineExtension` — see `internal/nodemedic/controller/nhd_reconciler.go`) marked the case `Failed` while the agent was still working. The full window before `phase=Failed` is up to ~120 s (initial 60 s + extension). Writes arriving during the extension window land normally; only writes arriving after `phase=Failed` is set are suppressed by this rule. The structured stdout logs (NFR-3) are the only record of what the agent ultimately did — durable for the lifetime of the pod / log-rotation window per the constitution's hackathon-scope simplifications. |
...
- The agent has its own Deployment (`nodemedic-agent`), its own ServiceAccount, its own RBAC, its own Service (`nodemedic-agent.cf-monitoring.svc:8080`), its own resource limits, and its own restart/rollout lifecycle.
...
- **`POST /diagnose` MUST return its `202 Accepted` response within 1.5 s p99.** This *is* a real boundary, sourced from the controller's HTTP client (`internal/nodemedic/agentclient/client.go` on `hackathon-2026/scope2-controller`, per-attempt `Timeout: 1500 * time.Millisecond`). Slower responses surface to the controller as `ResultTimeout` and trigger up to 4 retry attempts on the 1s/2s/4s backoff schedule. The agent's enqueue path (FR-1) MUST be fast enough to sit comfortably inside this boundary; the diagnosis loop itself runs asynchronously after the response. FR-10 idempotency on `caseId` is structurally load-bearing here — controller-side retries can deliver the same payload up to 4 times within ~7 s.
...
- **The controller's deadline (Spec 001 FR-5, ~60 s on first attempt) bounds the user-visible case duration**, but it does not cancel the agent. Note: the controller has a one-shot deadline-extension retry (default `RetryDeadlineExtension=60 s`, `internal/nodemedic/controller/nhd_reconciler.go`) — if the agent misses the first 60 s deadline, the controller pushes the deadline forward by another 60 s and retries `POST /diagnose` once before going terminal `Failed`. **The full late-write race window can therefore be up to ~120 s, not ~60 s.** FR-12's "do not overwrite `Failed`" rule covers writes that arrive after the second window closes; writes that arrive during the second window land normally.
- **Soft targets** (used to size resource limits and observability dashboards, not to gate `Failed`/`Acted`):
  - `POST /diagnose` receipt → `202` returned: 1.5 s is the hard ceiling per the bullet above; queue-time well under that is healthy.
...
**Image registry path:** `cf-registry.nr-ops.net/container-fabric/nodemedic-agent` (per Clarifications). Mirrors the controller's `cf-registry.nr-ops.net/container-fabric/nodemedic-controller`. Push works with developer cf-registry credentials (verified empirically 2026-06-14 against the controller image). Tag convention: `dev-cf1z-<short-sha>` where `<short-sha>` is the 8-char prefix of the agent repo branch HEAD at build time, matching the controller's tagging convention.
...
- [ ] **AC-1** Apply CRD + install Helm chart on **cf1z** in the `cf-monitoring` namespace (per Clarifications binding — co-located with the controller and `hack-node-problem-detector`). `kubectl --context=cf1z get deploy nodemedic-agent -n cf-monitoring` shows 1/1 ready. `/healthz` and `/readyz` return 200. (`/readyz` includes the model-resolution check from FR-3 — the resolved primary + fallback model IDs MUST be in the startup log.)
...
- [ ] **AC-15** Runbook readiness checklist (gates AC-3): the canonical runbook at `prompts/runbook.md` (per Clarifications) MUST contain the seven binding clauses below, verified by a build-time lint (a `tests/check_runbook.sh` script in the agent repo, run by CI and by the Helm pre-install Job — exact wiring is plan-phase). The lint MUST exit non-zero and block AC-3 from being claimed if any clause is missing. Required clauses:
  ...
  3. **Recommendation rubric** — instructs the agent to choose `recommendation.action="NoAction"` when evidence is too thin to recommend cordoning (per FR-8). Runbook MUST clarify that `NoAction` is honored by the controller as a distinct outcome (`GateSkip`, per the controller's `confidence_gate.go`) — not as a gate-fail. The controller routes `NoAction` to a HumanInLoop Slack message with `reason: "recommendation.action=NoAction"` and skips cordon. This is the agent's explicit "I investigated, here's what I found, no automated action recommended" channel.
...
8. `kubectl --context=cf1z get nhd <name> -n cf-monitoring -o yaml` shows: `case` populated by controller, `diagnosis` populated by agent, `action` populated by controller. Three scopes, one CR, full trace.
9. `kubectl --context=cf1z logs <agent-pod> -n cf-monitoring --since=2m | jq 'select(.caseId=="<caseId>")'` — every probe shown with its full command string. (No durable JSONL for hackathon scope; the demo captures this output during the live run for the deck.)
