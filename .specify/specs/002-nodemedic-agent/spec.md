...
2. **NR MCP staging account stability.** Account `1` is shared. Confirm we won't hit a per-account NR MCP rate limit during back-to-back demo reruns. Mitigation if we do: stagger reruns, or have the runbook tell the agent to prefer cached `kubectl`/`Bash` evidence over re-querying NRQL. NR token is mounted as a Kubernetes Secret per NFR-6.
