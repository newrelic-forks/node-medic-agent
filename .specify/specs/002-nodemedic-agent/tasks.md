...
- **Article I.4 every tool call observable** — T056 (`PreToolUse` hook) + T036/T057 (case-complete log line in stub and real worker) + T077 (CI grep test that the hook is wired).
...
- [ ] T001 Confirm working tree is on `hackathon-2026/cf1z-baseline` and clean (`git status` reports nothing) before starting Phase 1; if a `hackathon-2026/scope3-agent` branch is wanted instead, cut it now from `cf1z-baseline` per plan.md and update tasks.md's working-tree note in the same commit.
- [ ] T002 Hand-scaffold the directory tree: `cmd/nodemedic-agent/`, `nodemedic_agent/{api,runner,tools,kube}/`, `prompts/`, `deployment/helm/nodemedic-agent/{templates,}/`, `tests/nodemedic_agent/{unit,integration,fixtures,helpers}/`. Each new Python package gets an empty `__init__.py` so `pytest --collect-only` exits 0 cleanly until Phase 2 lands real code.
- [ ] T003 Author `pyproject.toml` at repo root: `[project]` with name `nodemedic-agent`, Python 3.12 floor, deps from plan §Primary Dependencies (`claude-agent-sdk`, `fastapi`, `uvicorn[standard]`, `kubernetes`, `pydantic`, `pydantic-settings`, `httpx`, `structlog`, optional extra `metrics` for `prometheus-client` + `prometheus-fastapi-instrumentator`). `[tool.uv]` block with `dev-dependencies = ["pytest", "pytest-asyncio", "respx"]`. `[tool.pytest.ini_options]` with `testpaths = ["tests/nodemedic_agent"]` so pytest only picks up agent tests, leaving any future Go test discovery untouched. `[tool.setuptools.packages.find]` scoped to `["nodemedic_agent*"]`.
- [ ] T004 Run `uv lock` once to produce `uv.lock` at repo root (deterministic build input for `Dockerfile.nodemedic-agent`); commit both `pyproject.toml` and `uv.lock` in the same commit. Verify `uv sync --all-extras --dev` succeeds in a clean checkout.
- [ ] T005 Author `cmd/nodemedic-agent/main.py` as a thin uvicorn entrypoint that imports `nodemedic_agent.app:create_app` and calls `uvicorn.run(...)` with `--workers 1` baked in. Module is intentionally <30 lines — all wiring lives inside `nodemedic_agent.app`.
- [ ] T006 [P] Author `Dockerfile.nodemedic-agent` at repo root per research R-10: three-stage build (Python builder with `--platform=$BUILDPLATFORM`, CLI bundle stage with `--platform=$BUILDPLATFORM` for `aws` CLI v2 + `az` CLI 2.x + `kubectl` 1.33.x, runtime stage pinned to `linux/amd64` because cf1z is amd64-only). `COPY prompts/runbook.md /app/prompts/runbook.md` (image SHA pins the runbook version per Clarifications). `CMD ["/app/.venv/bin/uvicorn", "nodemedic_agent.app:app", "--host", "0.0.0.0", "--port", "8080", "--workers", "1"]`. Image label `org.opencontainers.image.source` points at the agent repo URL.
- [ ] T007 [P] Extend top-level `Makefile` with `nodemedic-agent-help`, `nodemedic-agent-lint`, `nodemedic-agent-test`, `nodemedic-agent-runbook-lint`, `nodemedic-agent-helm-lint`, `nodemedic-agent-docker-build`, `nodemedic-agent-docker-push`, `nodemedic-agent-clean`. Image tag convention `dev-cf1z-$(shell git rev-parse --short=8 HEAD)` matching the controller's tagging. Verify NPD's `make test` and the controller's `make nodemedic-*` targets are unchanged via `make -n test` and `make -n nodemedic-build` diff against the previous commit.
...
- [ ] T034 [P] [US1] Author `nodemedic_agent/api/health.py`: `GET /healthz` (always 200 if process is up); `GET /readyz` calls `model_resolver.resolve()` (T036) + an HTTP HEAD against `NR_MCP_URL` with `READYZ_PROBE_TIMEOUT_SEC` budget; caches the result for 30 s.
- [ ] T035 [P] [US1] Author `nodemedic_agent/runner/model_resolver.py` per data-model.md §6 + FR-3 fallback chain: `ResolvedModels` Pydantic type, `resolve(settings, http_client) -> ResolvedModels` async function that hits the gateway's catalog endpoint, walks the exact → best-Opus → best-Sonnet → fail chain. Logs `event=model_resolved primary=… fallback=… resolution_path=…` at INFO at startup.
...
- [ ] T052 [US2] Author `prompts/runbook.md` v1 — replace v0 placeholders with the real runbook content. Required sections (per AC-15):
  ...
  8. `## KubeletUnhealthy` decision tree as a stub heading only — full content in US3 (T067).
...
- [ ] T056 [P] [US2] Author `nodemedic_agent/runner/hooks.py`: `tool_log_hook(...)` per Article I.4 — emits a `ToolCallLog` JSON line on `PreToolUse`; structlog-formatted; latency / exit_code / stdout_bytes filled in via the `PostToolUse` hook variant if available. Truncates `command` to 2 KB and `args` to 2 KB.
- [ ] T057 [US2] Author the **real** `nodemedic_agent/runner/case_worker.py` — replaces the stub worker (T036). `async def run_case(case: Case, nhd_writer: NHDWriter, settings: Settings)`:
  ...
  2. Build `ClaudeAgentOptions(system_prompt=load_runbook(...), permission_mode="bypassPermissions", hooks=[("PreToolUse", tool_log_hook)], mcp_servers={"nodemedic": mcp_server, "nr": nr_mcp_config(settings)}, allowed_tools=["mcp__nodemedic__emit_report", "mcp__nr__*", "Bash", "Read", "Write", "Edit", "Glob", "Grep", "WebFetch", "WebSearch"])` with `model=resolved.primary_resolved`, `fallback_model=resolved.fallback_resolved`.
  3. `async with ClaudeSDKClient(options=options) as client: await client.query(build_user_prompt(case)); async for msg in client.receive_response(): ...`.
  4. Loop ends naturally on `emit_report` tool call (the tool function does the CR write and returns) or model halt. On exception → write `Failed` CR with `reason ∈ {ModelError, ToolError}` per FR-11.
  5. Emit `case_complete` log line last (per data-model.md §5).
  6. **No agent-side termination ceiling** (FR-7) — no `max_turns`, no `max_budget_usd`, no wall-clock cap.
...
- [ ] T071 [P] [US4] Author `tests/nodemedic_agent/integration/test_late_write_suppression.py` — drive the writer with a mock apiserver that returns `phase=Failed` on the pre-write read; assert `WriteOutcome.DEFERRED_PHASE_CONFLICT`, the deferred-write log line carries `case_id`, `observed_phase=Failed`, and the intended payload, the case-complete log line shows `final_phase=Diagnosed_deferred` (or whatever the agreed-upon field name in data-model.md §5 is — pick one and lock it here).
...
- **Phase 8 (Polish, T077–T084)**: Depends on all desired user stories.
