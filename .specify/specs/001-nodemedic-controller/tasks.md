...
- [x] T027 [P] [US1] Authored `internal/nodemedic/providerid/parse_test.go` — 9 cases including the EKS fixture round-trip, errors.Is sentinel checks, malformed segments.
- [x] T028 [P] [US1] Authored `internal/nodemedic/controller/confidence_gate_test.go` — 12 table cases covering each clause separately, the GateSkip (NoAction) branch, missing recommendation, and cumulative-failure reasons.
- [x] T029 [P] [US1] Authored `internal/nodemedic/controller/nameformat_test.go` — covers EKS dotted names, Azure VM names, uppercase normalisation, dot→dash sanitization, 50-char truncation, all-special-chars fallback, determinism.
- [x] T030 [P] [US1] Authored `internal/nodemedic/agentclient/client_test.go` — golden-file lock on the request JSON, retry tests for 429/5xx (3 retries 1/2/4s), no-retry on 400/401, 409→202 idempotency synthesis (research R-5), timeout-class classification, context-cancellation.
- [x] T031 [P] [US1] Authored `internal/nodemedic/notifier/slack_test.go` — golden Block Kit shape, kubectl block embedded (research R-6), audit-log section appears only when AuditLogRef is set, retry-on-5xx, no-retry-on-4xx, empty-webhook fail-fast.
- [x] T032 [US1] Authored `internal/nodemedic/controller/us1_envtest_test.go` — gated by `//go:build integration` so unit tests stay fast. Two tests: `TestUS1_HappyPath_GatePassCordons` (canned Diagnosed → cordon + Slack) and `TestUS1_PendingPhasePostsToAgent` (empty status → POST /diagnose → phase=Diagnosing). Run via `make nodemedic-envtest`.

### Implementation for User Story 1

- [x] T033 [P] [US1] Implemented `internal/nodemedic/providerid/parse.go` — `ParseAWS` with sentinel errors (`ErrEmpty`, `ErrUnsupportedScheme`, `ErrMalformed`); `azure://` is rejected here and lands properly in US4 / T065.
- [x] T034 [P] [US1] Implemented `internal/nodemedic/controller/confidence_gate.go` — pure `ConfidenceGate(d, minConf, minSources) GateResult`; emits combined `Reason` for cumulative failures so Slack triage gets all reasons, not just the first.
- [x] T035 [P] [US1] Implemented `internal/nodemedic/controller/nameformat.go` — sanitizes per data-model §1.5 (lowercase, replace non-`[a-z0-9-]` with `-`, trim trailing dashes, fall back to `node` if empty).
- [x] T036 [P] [US1] Implemented `internal/nodemedic/controller/debounce.go` — `sync.Map`-backed `DebounceMap`; `Allow(k, now)` returns false within window, denial preserves prior LastSeen so the next allow is keyed off the original anchor.
- [x] T037 [P] [US1] Implemented `internal/nodemedic/agentclient/{types,client}.go` — bearer-token auth, 1.5s per-attempt timeout, exp 1/2/4s backoff, structured `AttemptError{Class}` so the reconciler branches on errors.Is rather than string match. `ResultClass.String()` doubles as the metric label.
- [x] T038 [P] [US1] Implemented `internal/nodemedic/notifier/slack.go` — stdlib `net/http`, 5s per-attempt, FR-9 retry chain, `transientErr` wrapper for retryable failures.
- [x] T039 [P] [US1] Implemented `internal/nodemedic/notifier/messages.go` — `BuildApplied` produces `header → fields → rca → kubectl block → audit log` Block Kit; HumanInLoop + Critical builders deferred to US2/US3 with placeholder package comments.
- [x] T040 [US1] Implemented `internal/nodemedic/controller/cordon.go` — `Cordon(ctx, c, node, expectedNodeName)`; refuses if `node.Name` mismatches with `ErrWrongNode`; idempotent on already-cordoned. Fake-client tests cover all four paths.
- [x] T041 [US1] Implemented `internal/nodemedic/controller/nodewatcher.go` — controller-runtime Reconciler keyed on Node, gated by `transitionPredicate` that compares old/new conditions and only enqueues on a watched-type transition to True (Create events also fire if a watched True is already on the Node, covering controller startup).
- [x] T042 [US1] Implemented `internal/nodemedic/controller/case_creator.go` — `CreateCase` builds the NHD with deterministic name + UUIDv4 caseId, calls `Create` with `IgnoreAlreadyExists`-equivalent (gets the existing object on conflict), emits `MetadataResolutionError` typed errors so the watcher emits the right `MetadataResolutionFailed` Event.
- [x] T043 [US1] Implemented `internal/nodemedic/controller/nhd_reconciler.go` — phase machine with US1 arms wired (`""→Diagnosing`, `Diagnosed→Acted` via gate-pass-or-fail), `Diagnosing→Failed` on deadline (US3 will add retry-once + Critical Slack), terminal phases short-circuit. Status updates use the status subresource via `client.MergeFrom`. Phase transitions update `status.conditions[]` per data-model §1.4 and emit `PhaseTransition` Events.
- [x] T044 [US1] Wired both reconcilers into `cmd/nodemedic-controller/main.go`; agent + Slack clients sourced from env vars (`NODEMEDIC_AGENT_TOKEN`, `NODEMEDIC_SLACK_WEBHOOK_URL`); added `--namespace` flag (default `cf-monitoring`); metrics registered via `metrics.MustRegister()` before `mgr.Start`.
- [x] T045 [P] [US1] Authored `deployment/helm/nodemedic-controller/values-eks.yaml` — pinned `agentURL` + image tag; `clusterName` deliberately NOT defaulted so each install passes it explicitly (per-install audit). `helm-lint` confirms it renders against `--set clusterName=test-odd-wire`.
- [x] T046 [US1] Added "NodeMedic controller (Scope 2)" section at the top of `README.md` covering layout, build/test targets, install flow, and the link to `.specify/specs/001-nodemedic-controller/quickstart.md`. NPD's existing README content is preserved below.
- [ ] T047 [US1] **MANUAL — runs at hackathon time on real cluster.** Run [`quickstart.md`](./quickstart.md) Scenario A end-to-end on `test-odd-wire` against the T026 canned NHD; capture `kubectl get nhd -o yaml` + `kubectl describe node` showing `SchedulingDisabled`; verify AC-1, AC-2, AC-3.
- [ ] T048 [US1] **MANUAL — runs at hackathon time on real cluster.** Quickstart Scenarios D (debounce, AC-6) + G (restart, AC-9).
- [ ] T049 [US1] **MANUAL — runs at hackathon time on real cluster.** Quickstart Scenario H (metrics, AC-10) — `nodemedic_cases_total{outcome="Applied"} >= 1`.
